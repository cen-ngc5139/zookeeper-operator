...
var log = ctrl.Log.WithName("controllers").WithName("finalizer")
...
func (h *Handler) reconcileFinalizers(finalizers []Finalizer, object metav1.Object) bool {
	...
	for _, finalizer := range finalizers {
		...
		if !utils.StringInSlice(finalizer.Name, object.GetFinalizers()) {
...
func (h *Handler) executeFinalizers(finalizers []Finalizer, object metav1.Object) (bool, error) {
	...
	for _, finalizer := range finalizers {
		...
		if utils.StringInSlice(finalizer.Name, object.GetFinalizers()) {
			...
			needUpdate = true
			object.SetFinalizers(utils.RemoveStringInSlice(finalizer.Name, object.GetFinalizers()))
		}
	}
