...
func (s *Scale) DownScale() error {
	...
	if expectReplica < actualReplica {
		...
		s.Workload.Status.Phase = cachev1alpha1.ZooKeeperDownScaling
		msg = fmt.Sprintf(model.MessageZooKeeperDownScaling, actualReplica, expectReplica)
		s.Recorder.Event(s.Workload, corev1.EventTypeNormal, model.ZooKeeperDownScaling, msg)
